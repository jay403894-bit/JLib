...
points, the start of the steal scan, and the two inbox-found points. Wired into CI the same way
`JLIBSCHED_STEAL_STATS` is: one build-and-run on the cheapest runner, so the diagnostic can't
silently bit-rot between the rare times someone actually needs it.
